...
社区已经有更深层需求的明确信号：GitHub issue 里出现"想看 orchestrator vs sub-agent 各花了多少 / 便宜模型是否承担了大部分工作"等问题，但没有产品在解决。
...
| 角色 | 定义 |
|------|------|
| `system_prompt` | system 字段（含 Claude Code 内置 system instructions） |
| `tool_definitions` | tools 数组中所有工具的 schema |
| `tool_results` | messages 中所有 tool_result block（历史 + 当前轮次） |
| `history` | messages 中除 tool_result 外的历史 user/assistant 消息 |
| `user_input` | 本轮新增的 user message |
| `assistant_output` | 本次响应的 output token |
| `cache_read` / `cache_write` | API 返回的 usage 中的 cache_read_input_tokens / cache_creation_input_tokens |

**实现要点**:
- 用本地 tokenizer（tiktoken / Claude 的分词器近似）对请求体各段分别计数
- 与 API 返回的 usage 数字交叉校验（已知总 input、cache_read、cache_write），用 7 类各段之和反推未上报的归因
- 误差在 ±5% 以内（与 usage 总数对齐）
...
### FR-AG-4: 检测器 D1–D5（首发 5 个）

**优先级**: P0
**状态**: proposed

每个检测器输出一条 finding：`{id, severity, title, evidence, estimated_savings_usd, fix_path}`。

| ID | 检测器 | 检测逻辑 | severity 判定 | 修复路径 |
|----|--------|---------|---------------|---------|
| **D1** | 工具定义膨胀 | 统计 session 内已加载 vs 实际调用过的工具，闲置工具的 tool_definitions token × 平均出现轮次 × input 单价 | 闲置 > 50% 且总损耗 > $0.50 = HIGH | ⚡ synrouter 动态裁剪 |
| **D2** | 缓存前缀破坏 | 对比相邻请求的 prefix（按 token 流），定位首个分叉点；若分叉点在前 5% 内 → 几乎全部缓存写入失效 | 单 session 损失 > $1.0 = HIGH | ⚡ synrouter 断点管理 |
| **D3** | 重复工具结果 | 对每条 tool_result 内容做 SHA256 哈希 + size 桶；同一哈希出现 ≥ 3 次 = 重复回传；统计重复部分 token × input 单价 × 出现轮次 | 重复占比 > 20% = MED | 手动（用户改 prompt 或 retrieval）+ synrouter 去重 |
| **D4** | 超长工具结果 | 单次 tool_result > 10K tokens 且后续 N 轮未被引用（被引用 = 在后续 assistant 消息中出现 ≥ 30 token 子串重合） | 单条 > 20K = MED | 手动（agent 端配 max-output） |
| **D5** | 可压缩历史 | 早期 turn (第 1 到第 K，K=总轮次 × 20%) 在最后 5 轮的 assistant 上下文中零引用 | 可压缩 token > 总 history 的 30% = LOW | 手动（compact 时机） |
...
{
  "version": "0.1.0",
  "session": { "agent": "claude-code", "project": "my-project", "turns": 42, "duration_sec": 4980 },
  "model": "claude-sonnet-4-5",
  "cost_usd": 4.83,
  "potential_savings_usd": 2.91,
  "tokens": { "input": 3042100, "output": 118300 },
  "cache": { "hit_rate": 0.38, "achievable": 0.85 },
  "composition": {
    "tool_results":    { "tokens": 1240000, "pct": 0.41, "cost_usd": 1.86 },
    "tool_definitions":{ "tokens":  680000, "pct": 0.22, "cost_usd": 1.02 },
    "history":         { "tokens":  520000, "pct": 0.17, "cost_usd": 0.78 },
    "system_prompt":   { "tokens":  390000, "pct": 0.13, "cost_usd": 0.59 },
...
| Tokenizer | `@anthropic-ai/tokenizer` + `js-tiktoken` | tiktoken-node（需编译 binding） |
...
│   │   └── index.ts        # 注册表
│   ├── render/
│   │   ├── terminal.tsx    # 默认终端报告（picocolors）
│   │   ├── html.ts         # --html 单文件输出
│   │   └── json.ts         # --json 输出
│   ├── tui/                # v0.3：Ink components
│   │   └── watch.tsx
│   └── lib/                # tokenizer / glob / fs 工具
├── tests/                  # vitest
├── assets/
│   └── pricing.json        # 内置定价快照
├── package.json
...
v0.1 只发布 5 个，但需要为后续保留扩展位。每加一个检测器 = 一篇技术博客 + 一次社区话题。

候选清单（按优先级）：

| 候选 | 检测内容 | 与 synrouter 关联 |
|------|---------|----------|
| **D6** | system_prompt 动态片段定位（时间戳、文件列表、git status 等导致每轮缓存失效） | ⚡ synrouter cache prefix stabilizer |
| **D7** | tool_definitions 顺序抖动（同一 MCP server 重启后顺序变化破坏缓存） | ⚡ synrouter tool order canonicalizer |
| **D8** | subagent 派生导致 prefix 分叉（Claude Code 的 Task tool 派生子 session） | ⚡ synrouter subagent-aware caching |
| **D9** | 模型选择失配（Sonnet 在做 Haiku 能搞定的活） | 信息性（synrouter 暂不做路由优化） |
| **D10** | 重试风暴（同一 turn 内多次失败重试） | 信息性 |
...
| D6 system_prompt 动态片段 | prefix stabilizer | （新规划） |
| D7 tool_definitions 顺序抖动 | tool order canonicalizer | SPEC-012 agent-tool-format-adaptation |
...
| **T+2 w** | Week 2 | 7 类归因 + D1-D3 检测器 + 终端报告输出 | alpha |
...
| **T+8 w** | Week 8 | 代理模式（FR-AG-10） | v0.2.0 |
...
| Q2 | Anthropic tokenizer 用哪个库 | `@anthropic-ai/tokenizer`（官方）+ tiktoken 做 OpenAI 后续 |
